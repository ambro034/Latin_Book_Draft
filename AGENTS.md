...
Local tooling secrets (`NEON_DATABASE_URL`, `OPENROUTER_API_KEY`,
`OPENROUTER_MODEL`, `SUBSTACK_COOKIES_STRING`, `SUBSTACK_EMAIL`,
`SUBSTACK_PUBLICATION_URL`) live in the cloud in Doppler, project `beops`,
config `dev`. Run any tool that needs them via `doppler run -- <cmd>`, which
injects them as env vars (no plaintext `.env` required). The repo-committed
`doppler.yaml` pins the project/config.

Set up on a new machine:
```bash
brew install dopplerhq/cli/doppler   # or see docs.doppler.com
doppler login                        # browser auth
doppler setup --no-interactive       # reads doppler.yaml -> beops/dev
doppler run -- python3 posts-generator/substack_drafter.py "_posts/<file>.md" -k 6
```
Update a secret: `doppler secrets set NAME -p beops -c dev` (e.g. when the
Substack cookie expires). A local gitignored `posts-generator/.env` still works
as a fallback (the scripts auto-load it), but Doppler is the source of truth and
the only one that syncs across machines. GitHub Actions secrets are separate and
unreadable; keep them in sync manually if a CI workflow ever needs the value.
...
- **Supported categories**: `devops`, `k8s`, `sre`, `ai`, `job-interviews`. Single source of truth is `_data/categories.yml`. Each has an index page in `_pages/<slug>.md` with `permalink: /<slug>/` and `is_category: true` in front matter. Adding a new category requires (1) appending to `_data/categories.yml`, (2) creating `_pages/<slug>.md`, (3) updating `BEOPS_CATEGORIES` in `.github/workflows/site-audit.yml`, and (4) mapping the slug in `CATEGORY_MAP` in `.github/scripts/build_posts_ts.py` so the post syncs to `beops.site` (unmapped categories are silently dropped from the external site).
- **External site sync (`beops.site`)**: the React site `NeverTheSame/beops-main-site` is regenerated by `.github/workflows/sync-to-beops-site.yml` on every push to `gh-pages`. It only has the filter tabs `devops | kubernetes | ai | interviews`, so new sub-categories must be mapped onto one of those in `build_posts_ts.py` (e.g. mapping a new `ai-foo` slug to `ai`). The sync also skips weekends in UTC unless run via `workflow_dispatch`.
- **Telegram draft (`@beops_it`)**: `propose-telegram-post.yml` fires on push to `gh-pages` touching `_posts/**`. It runs `posts-generator/telegram_drafter.py`, which ALWAYS drafts in Russian, imitates the author's voice from the `rag.style` corpus, links one related prior channel post (interconnection), and DMs the draft via the bot for manual review. Nothing auto-posts.
- **Substack draft (`beops.substack.com`)** is a LOCAL-ONLY step, not a CI workflow. Substack has no official API and its Cloudflare bot-protection blocks GitHub Actions datacenter IPs (returns a `403` JS challenge regardless of cookie validity), so the draft must be generated from a machine with a residential IP. Run `posts-generator/substack_drafter.py` locally after a post is published; it rewrites the post SHORTER in English (keeping all technical detail), ends with a backlink to the canonical BeOps post, and creates a Substack DRAFT (never auto-publishes) via the unofficial `python-substack` client. The draft is idempotent by title so re-runs don't duplicate. Secrets come from Doppler (see "Secrets" below); the local command is:
  ```bash
  cd posts-generator
  doppler run -- python3 substack_drafter.py "../_posts/<file>.md" -k 6
  ```
  Review and publish the draft manually in the Substack dashboard. If the run
  prints `401 Please sign in`, the `substack.sid` cookie expired: grab a fresh
  one and update it with `doppler secrets set SUBSTACK_COOKIES_STRING -p beops -c dev`.
