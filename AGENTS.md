...
- **Substack draft (`beops.substack.com`)**: `propose-substack-post.yml` fires on push to `gh-pages` touching `_posts/**`. It runs `posts-generator/substack_drafter.py`, which rewrites the post SHORTER in English (keeping all technical detail), ends with a backlink to the canonical BeOps post, and creates a Substack DRAFT (never auto-publishes) via the unofficial `python-substack` client. Substack has no official API, so auth is by secret: set `SUBSTACK_COOKIES_STRING` (preferred, copy the cookie header from a logged-in browser) OR `SUBSTACK_EMAIL` + `SUBSTACK_PASSWORD`, plus the `SUBSTACK_PUBLICATION_URL` repo variable (defaults to `https://beops.substack.com`). The draft is idempotent by title so re-runs don't duplicate.
